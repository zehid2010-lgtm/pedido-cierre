...
export async function traerCruce(): Promise<Cruce | null> {
  const [snapshot, equivalencias] = await Promise.all([
  ...
  const clientes = snapshot.clientes.map((c) => {
    ...
    for (const det of detallePorCliente.get(c.cliente) ?? []) {
      ...
      if (unidadesPorPack && unidadesPorPack > 0) packs += det.faltante / unidadesPorPack;
      else pendiente = true;
    }
  ...
  const cumplimientoGeneral =
    sugeridoTotal > 0 ? Math.min((compradoTotal / sugeridoTotal) * 100, 100) : null;
