  ...
  head: () => ({
    meta: [
      { title: "Clientes — Pedido Sugerido Tucumán" },
      {
        name: "description",
        content:
          "Listado de clientes con cumplimiento, sugerido, comprado y faltante en unidades y cajas.",
      },
      { property: "og:title", content: "Clientes — Pedido Sugerido Tucumán" },
      {
        property: "og:description",
        content: "Cumplimiento, sugerido, comprado y faltante por cliente.",
      },
    ],
  }),
  component: Clientes,
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {(["todos", "critico", "amarillo", "verde", "ambiguo"] as const).map(
              (e) => (
                <button
                  key={e}
                  onClick={() => setEstado(e)}
            ...
            {filtrados.map((c) => (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                  <Dato
                    titulo="Sugerido"
                    valor={nf.format(Math.round(c.sugerido))}
                  />
                  <Dato
                    titulo="Comprado"
                    valor={nf.format(Math.round(c.comprado))}
                  />
                  <Dato
                    titulo="Faltante"
                    valor={nf.format(Math.round(c.faltante))}
                    destacado
                  />
                </div>
                ...
                <p className="mt-2 text-center text-xs text-muted-foreground">
                  {c.faltantePacks !== null
                    ? `≈ ${nf1.format(c.faltantePacks)} cajas`
                    : "Sin equivalencia de cajas"}
                  {c.equivalenciaPendiente
  ...
  return (
    <div
      className={
        ...
        (destacado ? "bg-critico-soft" : "bg-muted")
      }
    >
      ...
      <p
        className={
          "numero-tabular text-sm font-bold " +
          (destacado ? "text-critico" : "")
        }
      >
        {valor}
      </p>
