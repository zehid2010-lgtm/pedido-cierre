...
WITH ultima_importacion AS (
  SELECT id FROM importaciones WHERE estado = 'procesada' ORDER BY created_at DESC LIMIT 1
), fecha_actual AS (
  SELECT max(fecha_venta) AS fecha FROM compras_semanales
), pedido_clientes AS (
  SELECT DISTINCT d.cliente, d.razon_social,
  ...
  SELECT DISTINCT c.cliente, c.razon_social,
  ...
  SELECT p.cliente AS cliente_pedido,
    (SELECT count(DISTINCT vn.cliente) FROM ventas_clientes vn
      WHERE vn.nombre_norm = p.nombre_norm AND p.ruta_norm IS NOT NULL AND vn.ruta_norm IS NOT NULL AND vn.ruta_norm = p.ruta_norm) AS n_candidatos,
    (SELECT min(vn.cliente) FROM ventas_clientes vn
      WHERE vn.nombre_norm = p.nombre_norm AND p.ruta_norm IS NOT NULL AND vn.ruta_norm IS NOT NULL AND vn.ruta_norm = p.ruta_norm) AS cliente_candidato,
  ...
  SELECT c.cliente_pedido,
    CASE WHEN c.hay_exacto THEN c.cliente_pedido WHEN c.n_candidatos = 1 THEN c.cliente_candidato ELSE NULL END AS cliente_venta,
    CASE WHEN c.hay_exacto THEN 'ID_EXACTO' WHEN c.n_candidatos = 1 THEN 'NOMBRE_UNICO' WHEN c.n_candidatos = 0 THEN 'SIN_COMPRA' ELSE 'AMBIGUO' END AS estado_cruce_cliente
  FROM candidatos c
), pedido AS (
  ...
  FROM detalle_mpr d
  LEFT JOIN resolucion_cliente r ON r.cliente_pedido = d.cliente
  WHERE d.importacion_id = (SELECT id FROM ultima_importacion)
), ventas AS (
  SELECT c.cliente, c.mpr, c.venta_co,
    ...
    CASE
      WHEN upper(c.mpr) LIKE '%NO RETORNABLE%' THEN 'NR'
      ...
      WHEN upper(c.mpr) ~ '(^|[^A-Z0-9])(TB|TETRA|TETRABRIK|TETRA BRIK|LATA|LATAS|PET)([^A-Z0-9]|$)' THEN 'NR'
      ELSE 'SIN_DATO'
    END AS retorno,
  ...
  LEFT JOIN ventas v ON p.cliente_venta IS NOT NULL AND v.cliente = p.cliente_venta AND v.marca = p.marca
    AND v.volumen_ml = p.volumen_ml AND v.variante = p.variante AND v.retorno = p.retorno
    AND v.es_fresh = p.es_fresh AND (p.es_soda_benedictino = false OR v.con_gas = true)
...
ALTER VIEW public.vw_pedido_sugerido_actual SET (security_invoker = true);
GRANT SELECT ON public.vw_pedido_sugerido_actual TO authenticated;